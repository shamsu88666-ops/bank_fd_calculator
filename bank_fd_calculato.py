    ...
    if start_date and end_date and principal > 0 and interest_rate > 0:
        ...
        if total_days > 0:
            ...
            with pd.ExcelWriter(buffer, engine='xlsxwriter') as writer:
                ...
                # Professional Formatting with Center Alignment and New Color Theme
                ...
                # Formatting columns to center content
                worksheet.set_column('A:A', 25, cell_format)
                worksheet.set_column('B:B', 20, cell_format)
                
                # Overwriting data with centered format
                for row_num, (part, det) in enumerate(zip(excel_data["Particulars"], excel_data["Details"])):
                    worksheet.write(row_num + 1, 0, part, cell_format)
                    worksheet.write(row_num + 1, 1, det, cell_format)
